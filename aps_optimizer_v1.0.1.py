...
Comp=pulp.LpVariable.dicts("compDate",(prod_line,orderList),0)
...
for o in orderList:
    for l in prod_line:
        prob+=r[l][o]+Comp[l][o] <= (orderPool['deli_date'][o]-datetime.datetime.strptime(plan_dates[0],'%Y-%m-%d')).days#reference the first day of plan-dates as the first day
    ...
    prob+=pulp.lpSum([new_df[(new_df['order_id']==o)&(new_df['line_no']==l)]['spd_15']*Comp[l][o] for l in prod_line])==orderPool['order_num'][o]
    #prob+=pulp.lpSum([new_df[(new_df['order_id']==o)&(new_df['line_no']==l)]['spd']*Comp[l][o] for l in prod_line])<=orderPool['order_num'][o]

for l in prod_line:
    prob+=pulp.lpSum([Comp[l][o] for o in orderList if r[l][o]<=len(plan_dates)])>=len(plan_dates)
