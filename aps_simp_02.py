...
model1Pool,model2Pool=prep.pools_1linedays(modelPool.reset_index(),dp_matrix)
...
model2list=model2lines['model_no'].unique().tolist()
k_comb={}
for i in model2list:
    tempL=[]
    loop_loop=model2lines[model2lines['model_no']==i]['line_no'].tolist()
    for l in loop_loop:
...
for j in model2list:
    k[j]=pulp.LpVariable.dicts('k',k_comb[j],0,1,pulp.LpContinuous)
    f[j]=pulp.LpVariable.dicts('f',f_comb[j],0,1,pulp.LpContinuous)
    wt[j]=pulp.LpVariable.dicts('w',w_comb[j],0,1,cat='Binary')        
...
for d in model2list:
    prob+=pulp.lpSum([f[d][(d,l)] for l in model2lines[model2lines['model_no']==d]['line_no'].tolist()])==1
    for l1 in model2lines[model2lines['model_no']==d]['line_no'].tolist():
        prob+=pulp.lpSum([k[d][(d,l1,m)] for m in P[d][l1][0]])==1 
        prob+=pulp.lpSum([wt[d][(d,l1,m)] for m in P[d][l1][0]])==1
        prob+=pulp.lpSum([k[d][(d,l1,m)]*P[d][l1][1][m] for m in P[d][l1][0]])==f[d][(d,l1)]*modelSum[d] 
        prob+=pulp.lpSum([ft[d][(d,l1,t)] for t in Tn[d]])<=1
        prob+=f[d][(d,l1)]<=Y*pulp.lpSum([ft[d][(d,l1,t)] for t in Tn[d]])
        prob+=pulp.lpSum([ft[d][(d,l1,t)] for t in Tn[d]])<=f[d][(d,l1)]*Y
        prob+=pulp.lpSum([ft[d][(d,l1,t)] for t in Tn[d]])>=f[d][(d,l1)]
        
        #下面用于决定k必须是相邻两个部位0的值
        temp_loop=len(P[d][l1][0])
        for p in range(temp_loop):
            if (p !=0) and (p!=(temp_loop-1)):
                prob+=k[d][(d,l1,p)]<=wt[d][(d,l1,p-1)]+wt[d][(d,l1,p)]
            elif p==0:
                prob+=k[d][(d,l1,p)]<=wt[d][(d,l1,p)]
            elif p==temp_loop-1:
                prob+=k[d][(d,l1,p)]<=wt[d][(d,l1,p-1)]
        prob+=pulp.lpSum([k[d][(d,l1,m)]*P[d][l1][0][m] for m in P[d][l1][0]])+modelEP[d]<=pulp.lpSum([ft[d][(d,l1,t)]*t for t in Tn[d]])
        
        
        for b in model2lines[model2lines['line_no']==l1]['model_no']:
            if d!=b:
                prob+=z[d][(d,b,l1)]==1-z[b][(b,d,l1)]
                prob+=pulp.lpSum([ft[b][(b,l1,t)]*t for t in Tn[b]])+pulp.lpSum([k[d][(d,l1,m)]*P[d][l1][0][m] for m in P[d][l1][0]])-z[b][(b,d,l1)]*M<=pulp.lpSum([ft[d][(d,l1,t)]*t for t in Tn[d]])
                prob+=pulp.lpSum([ft[d][(d,l1,t)]*t for t in Tn[d]])+pulp.lpSum([k[b][(b,l1,m)]*P[b][l1][0][m] for m in P[b][l1][0]])-z[d][(d,b,l1)]*M<=pulp.lpSum([ft[b][(b,l1,t)]*t for t in Tn[b]])
        
model1list=model1Pool['model_no'].unique().tolist()
model1lines=model1Pool[['model_no','line_no']].drop_duplicates()
modelEpst=modelEP.reset_index()
model1Pool=model1Pool.merge(modelEpst,how='left',right_on='model_no',left_on='model_no')
model1Pool['lpst']=model1Pool['days']+model1Pool['absEpst']
for c in model1list:
    for l2 in model1lines[model1lines['model_no']==c]['line_no'].tolist():
        prob+=model1Pool[(model1Pool['model_no']==c) & (model1Pool['line_no']==l2)]['lpst'].item()<=pulp.lpSum([ft[c][(c,l2,t)]*t for t in Tn[c]])
        prob+=pulp.lpSum([ft[c][(c,l2,t)] for t in Tn[c]])==1
        
        for b in model1lines[model1lines['line_no']==l2]['model_no']:
            if c!=b:
                prob+=z[c][(c,b,l2)]==1-z[b][(b,c,l2)]
                prob+=pulp.lpSum([ft[b][(b,l2,t)]*t for t in Tn[b]])+model1Pool[(model1Pool['model_no']==c) & (model1Pool['line_no']==l2)]['days'].item()-z[b][(b,c,l2)]*M<=pulp.lpSum([ft[c][(c,l2,t)]*t for t in Tn[c]])
                prob+=pulp.lpSum([ft[c][(c,l2,t)]*t for t in Tn[c]])+model1Pool[(model1Pool['model_no']==b) & (model1Pool['line_no']==l2)]['days'].item()-z[c][(c,b,l2)]*M<=pulp.lpSum([ft[b][(b,l2,t)]*t for t in Tn[b]])
#for l in prod_line:
...
#rlt_df=pd.read_csv(r'C:\02_DataScience\02_codes\github\aps\test_24.csv')
...
model1Pool['model']=model1Pool['model_no'].apply(lambda x:x.replace('-','_'))
ft_df=kt_df.merge(model1Pool,how='left',left_on=['model','line'],right_on=['model','line_no'])
ft_df.drop(columns=['line_no'],inplace=True)
...
f_ped=vali_df[vali_df['label']=='f']
f_df=adt.kname_split(f_ped['cat'],False)

model2Pool['model']=model2Pool['model_no'].apply(lambda x:x.replace('-','_'))
f_ped=f_ped.merge(f_df,left_index=True,right_index=True,how='inner')
f_out=f_ped[['model','line','vvalue']].merge(model2Pool,how='left',left_on='model',right_on='model')
f_out['num']=f_out['vvalue']*f_out['order_num']

temp_ft=ft_df[ft_df['model_no'].isna()]

ft_multi=pd.DataFrame()
for index,item in temp_ft.iterrows():
    item[5]=k_out[(k_out['model']==item[0])&(k_out['line']==item[1])]['temp_t'].item()
    item[3]=model2Pool[model2Pool['model']==item[0]]['model_no'].item()
    item[4]=f_out[(f_out['model']==item[0])&(f_out['line']==item[1])]['num'].item()
    ft_multi=ft_multi.append(item,ignore_index=True)

out_df=ft_df[ft_df['model_no'].isna()==False].append(ft_multi,ignore_index=True)
#out_df.drop(columns=['line_no'],inplace=True)
out_df['release']=out_df['ped']-out_df['days']
...
#outome=pd.read_csv(r'C:\02_DataScience\02_codes\github\aps\aps\test_1.csv')
"""
out_df.to_csv('outcome.csv')
"""
...
numdf=out_df[['model_no','line','order_num','days']]
numdf['days']=numdf['days'].apply(round)
numdf=numdf.merge(modelEpst,how='left',left_on='model_no',right_on='model_no')
numdf['lpst']=numdf['days']+numdf['absEpst']

prob1=pulp.LpProblem('phase2',pulp.LpMinimize)
...
prob1+=pulp.lpSum([x[i][(i,j,t)]*(t-g[i][j]) for i in modelList for j in Md[i] for t in T[i][j] if t>g[i][j]])
...
for i in modelList:
    ...
    #限定完成只出现一次
    #限定h
    prob1+=pulp.lpSum([h[i][(i,t)] for t in Tm[i]])==1
    #限定f
    #prob+=pulp.lpSum([f[i][(i,l)] for l in model_line[model_line['model_no']==i]['line_no']])==1
    
    #限定订单总量即型号总量，x与h的关系
    #prob+=pulp.lpSum([x[i][(i,j,t)]*orderD[i][j] for j in Md[i] for t in T[i][j]])==pulp.lpSum([h[i][(i,t1)]*modelSum[i] for t1 in Tm[i]])
    for l in numdf[numdf['model_no']==i]['line']:
        #限定k的取值
        #prob+=pulp.lpSum([k[i][(i,l,m)] for m in P[i][l][0]])==1
        ...
        #限定ft
        prob1+=pulp.lpSum([ft[i][(i,l,t)] for t in Tn[i]])==1
        
        #k vs. f        
        #prob+=pulp.lpSum([k[i][(i,l,m)]*P[i][l][1][m] for m in P[i][l][0]])==f[i][(i,l)]*modelSum[i]
        #k与ft的关系/EPST 
        
        prob1+=pulp.lpSum([ft[i][(i,l,t)]*t for t in Tn[i]])>=numdf[(numdf['model_no']==i)&(numdf['line']==l)]['lpst'].item()
        ...
        for b in model_line[model_line['line_no']==l]['model_no']:
            if i!=b:
                prob1+=z[i][(i,b,l)]==1-z[b][(b,i,l)]
                prob1+=pulp.lpSum([ft[b][(b,l,t)]*t for t in Tn[b]])+numdf[(numdf['model_no']==i)&(numdf['line']==l)]['days'].item()-z[b][(b,i,l)]*M<=pulp.lpSum([ft[i][(i,l,t)]*t for t in Tn[i]])
                prob1+=pulp.lpSum([ft[i][(i,l,t)]*t for t in Tn[i]])+numdf[(numdf['model_no']==b)&(numdf['line']==l)]['days'].item()-z[i][(i,b,l)]*M<=pulp.lpSum([ft[b][(b,l,t)]*t for t in Tn[b]])
        
    for j in Md[i]:
        #限定x=1只能出现一次
        prob1+=pulp.lpSum([x[i][(i,j,t)] for t in T[i][j]])==1
        #prob+=pulp.lpSum([x[i][(i,j,t)]*t4 for t4 in T[i][j]])<=pulp.lpSum([h[i][(i,t8)]*t8 for t8 in Tm[i]])
        #缩小x出现的时间点
        #prob+=pulp.lpSum([x[i][(i,j,t)] for t in T[i][j] if t<oe[i][j]])==0
    ...
    for t3 in Tm[i]:
        ...
        prob1+=pulp.lpSum([x[i][(i,j,t4)] for j in Md[i] for t4 in T[i][j] if t4<=t3])>=h[i][(i,t3)]*len(Md[i])
        
        #限定ft 与h 的关系：
        prob1+=pulp.lpSum([ft[i][(i,l,t5)] for l in model_line[model_line['model_no']==i]['line_no'] for t5 in Tn[i] if t5<=t3])>=h[i][(i,t3)]*len(model_line[model_line['model_no']==i]['line_no'])

prob1.writeLP("APS_2.lp")
#solver=pulp.solvers.COIN_CMD(path='/home/yumchaamax/math_dep/cbc/Cbc-2.9/build/Cbc/src/cbc',threads=16,msg=1,fracGap=0.01)
prob1.solve(pulp.CPLEX())
...
for v in prob1.variables():
    #print(v.name, "=", v.varValue)
    a.append((v.name,v.varValue))
rlt_df1=pd.DataFrame(a)

rlt_df1.to_csv("test-2.csv")

#rlt_df1=pd.read_csv(r'C:\02_DataScience\02_codes\github\aps\test-2.csv')

rlt_df1.columns=['vname','vvalue']
vali_df1=rlt_df1[rlt_df1['vvalue']!=0]

temp_S1=vali_df1['vname'].str.split(pat='_\(')
vali_df1['label']=temp_S1.apply(lambda x:x[0])
vali_df1['cat']=temp_S1.apply(lambda x:x[1])

line_rls1=vali_df1[vali_df1['label']=='ft']
kt_df1=adt.kname_split(line_rls1['cat'])

#model1Pool['model']=model1Pool['model_no'].apply(lambda x:x.replace('-','_'))
#ft_df=kt_df1.merge(model1Pool,how='left',left_on=['model','line'],right_on=['model','line_no'])
#ft_df.drop(columns=['line_no'],inplace=True)
ft_out1=kt_df1.merge(out_df,how='left',left_on=['model','line'],right_on=['model','line'])
ft_out1=ft_out1.drop(columns=['ped_y','release'])
ft_out1['release']=ft_out1['ped_x']-ft_out1['days']
ft_out1.to_csv('final_ft.csv')   
print("Total amounts of products by day =", pulp.value(prob1.objective))     
